...
module.exports = {
    async getVendas(req, res, next) {
        try {
            const { id } = req.params
            const venda = await knex('vendas').join('users as comprador', 'comprador.id', '=', 'vendas.comprador_id').join('users as vendedor', 'vendedor.id', '=', 'vendas.vendedor_id').join('produtos as produto', 'produto.id', '=', 'vendas.produto_id').select('vendas.*', 'comprador.nome as comprador_nome', 'comprador.profpic as comprador_imagem', 'vendedor.nome as vendedor_nome', 'vendedor.profpic as vendedor_imagem', 'produto.nome as produto_nome', 'produto.imagem as produto_imagem').where('comprador_id', id).orWhere('vendedor_id', id).orderBy('created_at', 'desc')
